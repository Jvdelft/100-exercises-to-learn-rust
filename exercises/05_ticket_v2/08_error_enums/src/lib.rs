...
#[derive(Debug)]
enum TicketNewError {
    DescriptionError { err_msg: String },
    TitleError { err_msg: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionError { err_msg }) => {
            Ticket::new(title, "Description not provided".to_string(), status).unwrap()
        }
        Err(TicketNewError::TitleError { err_msg }) => panic!("{err_msg}"),
    }
}
